...
class SonyJoystick : public unitree::common::UnitreeJoystick
{
public:
    SonyJoystick(std::string device, int bits = 15)
    : unitree::common::UnitreeJoystick()
    {
        js_ = std::make_unique<Joystick>(device);
        if(!js_->isFound()) {
    ...
    void update() override
    {
        ...
        LB(js_->button_[4]);       // L1
        RB(js_->button_[5]);       // R1
        A(js_->button_[0]);        // Cross
        B(js_->button_[1]);        // Circle
        X(js_->button_[2]);        // Square
        Y(js_->button_[3]);        // Triangle
        up(js_->axis_[7] < -16384);
        down(js_->axis_[7] > 16384);
        left(js_->axis_[6] < -16384);
        right(js_->axis_[6] > 16384);
        LT(js_->axis_[2] < -16384);
        RT(js_->axis_[5] < -16384);
        lx(double(js_->axis_[0]) / max_value_);
        ly(-double(js_->axis_[1]) / max_value_);
        rx(double(js_->axis_[3]) / max_value_);
        ry(-double(js_->axis_[4]) / max_value_);
